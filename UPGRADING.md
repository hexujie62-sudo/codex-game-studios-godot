...
**New files to add:**
```
.agents/skills/art-bible/SKILL.md
.agents/skills/asset-spec/SKILL.md
...
**Existing files to overwrite (no user content):**
- All files under `.agents/skills/` modified in the commit range (skill audit + AskUserQuestion widgets + `--review` argument-hints)
- `.codex/hooks/log-agent.sh` (fix #21)
...
.agents/skills/create-architecture/SKILL.md
...
**New files to add:**
```
.agents/skills/consistency-check/SKILL.md
```

**Existing files to overwrite (no user content):**
```
.agents/skills/team-combat/SKILL.md      ← no-arg guard, verdict keywords, gate improvements
.agents/skills/team-narrative/SKILL.md   ← no-arg guard, verdict keywords, gate improvements
.agents/skills/team-ui/SKILL.md          ← no-arg guard, verdict keywords, gate improvements
.agents/skills/team-release/SKILL.md     ← no-arg guard, verdict keywords, NO-GO path
.agents/skills/team-polish/SKILL.md      ← no-arg guard, verdict keywords, gate improvements
.agents/skills/team-audio/SKILL.md       ← no-arg guard, verdict keywords, gate improvements
.agents/skills/team-level/SKILL.md       ← no-arg guard, verdict keywords, adjacent area checks
.agents/skills/team-live-ops/SKILL.md    ← no-arg guard, verdict keywords, ethics enforcement
.agents/skills/team-qa/SKILL.md          ← no-arg guard, verdict keywords, gate improvements
.agents/skills/map-systems/SKILL.md      ← verdict keywords
.agents/skills/create-epics/SKILL.md     ← "May I write" protocol fix, verdict keywords
.agents/skills/create-stories/SKILL.md   ← verdict keywords
...
.agents/skills/create-architecture/SKILL.md
.agents/skills/create-control-manifest/SKILL.md
.agents/skills/create-epics/SKILL.md
.agents/skills/create-stories/SKILL.md
.agents/skills/dev-story/SKILL.md
.agents/skills/propagate-design-change/SKILL.md
.agents/skills/content-audit/SKILL.md
...
Four new hooks are registered in this version. If you haven't customized `.codex/hooks.json`, overwriting is safe. Otherwise, add the following hook entries manually:
...
- `validate-skill-change.sh` — `PostToolUse` event filtered to `.agents/skills/` writes
...
- `catalog` — checks that all skills in `.agents/skills/` are indexed in the catalog
...
Skills are now explicitly assigned to Haiku, Sonnet, or Opus tiers based on task complexity. Read-only status checks use Haiku; complex multi-document synthesis uses Opus; everything else defaults to Sonnet. Tier assignments are documented in `.codex/docs/coordination-rules.md`.
...
1. **Verify new hooks** are registered in `.codex/hooks.json` — check for all four: `log-agent-stop.sh`, `notify.sh`, `post-compact.sh`, `validate-skill-change.sh`.
...
| **New files** | `.codex/hooks.json` hook wiring |
...
The Codex-only port keeps hook wiring in `.codex/hooks.json` and does not ship a
separate status line script. If you are upgrading from a legacy version that
added a `statusLine` block, remove that stale block unless your local Codex
...
The current stage is tracked through project artifacts and can be pinned by
writing a stage name to `production/stage.txt`. `/help`,
`/project-stage-detect`, and the session hooks use that state to orient the next
step.
...
2. **Verify stage detection** by running `/help` or `/project-stage-detect` in
   a Codex session.
...
Check whether the new version adds any hook entries you want. The change was
minor (schema update). If you haven't customized `.codex/hooks.json`,
overwriting is safe.
