...
The Codex hook layer is intentionally small. It provides session context,
obvious route-gap reminders, dangerous-command protection, skill-change
reminders, and post-compaction recovery prompts. Stronger checks live in
`.githooks/` and CI.

| Hook | Trigger | What It Does |
|------|---------|-------------|
| `session-start.sh` | SessionStart | Adds compact project context as Codex JSON additionalContext |
| `detect-gaps-lite.sh` | SessionStart | Reports only obvious missing route artifacts |
| `dangerous-command-policy.sh` | PreToolUse | Blocks clearly destructive shell operations |
| `skill-change-reminder.sh` | PostToolUse | Advises `/skill-test` after `.agents/skills/` changes |
| `post-compact-restore.sh` | PostCompact | Emits JSON reminder to read `active.md` after compaction |
| `.githooks/pre-commit` | git commit | Blocks invalid JSON and warns on design/code/asset/skill issues |
| `.githooks/pre-push` | git push | Blocks direct pushes to protected branch names |
...
**Automatic recovery:** The `session-start.sh` hook points Codex to
`active.md` automatically. After compaction, `post-compact-restore.sh` emits a
JSON reminder to read the same file. Keep important state in
`production/session-state/active.md`; do not depend on compaction hooks to
copy long text back into the conversation.
