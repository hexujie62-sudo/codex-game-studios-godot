...
3. **Skill 体系相关规则迁到 `/skill-create-cfg` 内部验证。**
...
| Skills | 工作流审查和人机协作把关 | `/help`、`/skill-create-cfg`、`/asset-audit` | 中高：依赖显式调用和体系路由 |
...
| `validate-skill-change.sh` | 当前按 `file_path` 解析，Codex `apply_patch` 下不可靠。 | 改成解析 patch diff，提醒使用 `/skill-create-cfg` 做内部审计。 | Codex `PostToolUse` + `/skill-create-cfg` 内部验证 | 能发现大部分 Codex 文件改动，但不当硬门。 |
...
| `PostToolUse` | `skill-change-reminder.sh` | 解析 `apply_patch`，发现 `.agents/skills/*/SKILL.md` 改动后提醒使用 `/skill-create-cfg` 做内部审计。 |
...
- TODO/FIXME 负责人格式提醒。
- `.agents/skills/*/SKILL.md` 修改时提示使用 `/skill-create-cfg` 做内部审计。
...
| 规则 | 迁移到 |
|---|---|
| Skill 修改后必须有 catalog entry | `/skill-create-cfg` 内部审计 |
| Skill 修改后必须有 spec | `/skill-create-cfg` 内部审计 |
| Skill frontmatter 结构 | `/skill-create-cfg` 内部静态检查 |
| Skill 行为是否符合用途 | `/skill-create-cfg` 内部 spec 检查 |
...
6. 增强 `/skill-create-cfg` 内部审计，覆盖 catalog/spec。
7. 增加 CI 检查。
8. 跑验收标准。
