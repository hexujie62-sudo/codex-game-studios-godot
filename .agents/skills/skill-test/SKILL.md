---
name: skill-test
description: "验证技能文件的结构合规性和行为正确性。四种模式：静态（linter）、规范（行为）、分类（rubric）、审计（覆盖率报告）。"
...
```
=== Skill Static Check: All 76 Skills ===
...
Also read `.codex/docs/skill-route-index.yaml` if present. This is the route
coverage source used by `/help`. It must not hard-code mutable design facts
such as 2D/3D, art style, camera, genre, core loop, input method, or platform as
stable constraints.
...
### Step 3a — Build Route Coverage Table

If `.codex/docs/skill-route-index.yaml` exists, verify:

- Every `.agents/skills/*/SKILL.md` skill has exactly one `skill_routes` entry.
- No `skill_routes` entry points to a missing skill directory.
- Every route entry has these fields: `name`, `stage`, `group`, `visibility`,
  `fit_basis`, `why`, `avoid_when`.
- `visibility` is one of: `core`, `support`, `explicit-only`.
- `stable_constraints` does not include mutable design details such as a specific
  art style, 2D/3D mode, camera, genre, input method, or platform.
- `mutable_design_fields` exists and lists the design facts that must be read
  from current artifacts instead of treated as permanent.

Summarize counts by `visibility`, `group`, and `fit_basis`. Flag missing fields
as FAIL because `/help` cannot explain its routing decision without them.
...
SKILLS (76 total)
Specs written: 76 (100%) | Never static tested: 76 | Never category tested: 76
...
ROUTE INDEX
Route entries: 76/76 skills covered
Visibility: core 25 | support 32 | explicit-only 19
Route field gaps: 0
Mutable design fields isolated: YES

Route gaps:
(none if every skill is covered and route entries have required fields)
...
Skill coverage:  76/76 specs (100%)
