---
name: map-systems
description: "将游戏概念分解为单独的系统，映射依赖关系，确定设计优先级，并创建系统索引。"
...
**After dependency mapping is approved, spawn `technical-director` via Task using gate TD-SYSTEM-BOUNDARY (`.codex/docs/director-gates.md`) before proceeding to priority assignment.**
...
**After priorities are approved, spawn `producer` via Task using gate PR-SCOPE (`.codex/docs/director-gates.md`) before writing the index.**
...
**After the systems index is written, spawn `creative-director` via Task using gate CD-SYSTEMS (`.codex/docs/director-gates.md`).**
