...
本 Skill 用来快速判断项目现在处在游戏开发流程的哪一步，并告诉用户下一步应该做什么。它是轻量导航，不是完整审计。需要完整缺口分析时，使用 `/project-stage-detect`。
...
- `visibility: core` 的 Skills 可以成为主推荐。
- `visibility: support` 的 Skills 只在当前阶段、blocker 或已检测缺口相关时作为辅助建议出现。
- `visibility: explicit-only` 的 Skills 默认隐藏，除非用户点名、Gate/report 明确要求，或项目明显处于对应例外场景。
...
如果 route index 存在，也要查询 uncataloged skill 的 route entry。根据 `visibility` 和 `why` 判断是否展示。除非用户措辞明确指向该 Skill，否则不要在 footer 中展示 `explicit-only`。
...
- 新建/恢复/接手: `/window-start-ccgs <lane-id>`
- 更新/交接: `/window-handoff-ccgs <lane-id>`
```
...
- 如果第一个 blocker 对应 `support`，仍然作为 required 显示，但说明它是 workflow catalog 要求，而不是宽泛默认推荐。
- 除非有直接证据，否则不要推荐 `explicit-only`。
- optional opportunities 中优先展示与当前 blocker 相关的 `support` Skills，不要让宽泛 team/release Skills 抢主推荐。
...
- 如果推荐 lane 未注册，显示：`建议新建窗口: /window-start-ccgs <lane-id>`。
- 如果当前窗口职责不匹配，提醒不要在当前窗口直接改对应文件。
- 如果任务跨多个 lane，先推荐 `A-producer` 做拆分和 owner 裁决。
- 如果发现 File conflicts，优先提示 `/window-handoff-ccgs audit`，不要继续并行写同一文件。

---

## Step 7: 检查 In-Progress Work
...
- 建议窗口: [registered lane id] 或 建议新建窗口: `/window-start-ccgs <lane-id>`
- 更新窗口: `/window-handoff-ccgs <lane-id>`
...
- 如果发现 File conflicts，必须提示先运行 `/window-handoff-ccgs audit`，不要继续并行改同一文件。
...
---
需要更详细的检查？
- `/project-stage-detect` — 完整 gap analysis，列出缺失工件。
