---
name: create-epics
description: "将已批准的 GDD 和架构转化为史诗——每个架构模块一个史诗。定义范围、管辖 ADR、引擎风险和未跟踪的需求。不分解为故事——在每个史诗创建后运行 /create-stories [epic-slug]。"
...
After all epics for the current layer are defined (Step 4 completed for all in-scope systems), and before writing any files, spawn `producer` via Task using gate **PR-EPIC** (`.codex/docs/director-gates.md`).
