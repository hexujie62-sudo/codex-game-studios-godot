...
更深的 QA、回归和证据检查由 B-dev/C-art 在具体 work order 中交付，阶段级判断由 `/gate-check` 处理。
...
runtime validation, CI/test command wiring, or when B-dev/gate-check reports
that the test foundation is missing.
...
这一步只建立测试基础，不替代具体 work order 中的 B-dev/C-art 证据交付。
...
- where B-dev/C-art should place domain evidence for work orders
...
- Next: /brainstorm, /design-system, /create-architecture, /gate-check, or a B-dev work order
