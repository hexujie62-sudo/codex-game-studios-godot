...
- 新增 Skill 或 Hook 前，先确认现有 `/window-start-ccgs`、`/window-handoff-ccgs`、`active.md`、lane 文件是否已经足够。
...
```text
Window commands:
- Start/create/recover: `/window-start-ccgs <lane-id>`
- Update/handoff: `/window-handoff-ccgs <lane-id>`
```
...
| 窗口 | 职责 | 主要文件 | 默认 Skill |
|---|---|---|---|
| A-producer | 总控、案子、范围、阶段、优先级、跨窗口冲突 | `production/session-state/active.md`、sprint、milestone、gate 报告 | `/help`、`/project-stage-detect`、`/sprint-plan`、`/gate-check` |
| B-dev | 开发、测试、代码实现、故事推进 | `src/`、`tests/`、`production/epics/**` | `/story-readiness`、`/dev-story`、`/code-review`、`/story-done` |
| C-art | 美术、资源、视觉规格、资源审计 | `design/art/`、`design/assets/`、`assets/` | `/art-bible`、`/asset-spec`、`/asset-audit` |
| D-qa | QA、缺陷、回归、测试证据 | `production/qa/`、`tests/`、test evidence | `/qa-plan`、`/smoke-check`、`/regression-suite`、`/test-evidence-review` |
| Z-platform | CCGS 底层、Skill、Hook、路由、文档、工具链 | `.agents/skills/`、`.codex/`、`.githooks/`、`docs/ccgs-*` | `/window-start-ccgs Z`、`/window-handoff-ccgs Z`、`/skill-create-ccgs`、`/skill-test` |
...
/help -> 当前阶段 + 当前问题类型 + Registered Windows
      -> 推荐 Skill + 推荐 lane
      -> 产物
      -> /window-handoff-ccgs update <lane-id>
      -> /window-handoff-ccgs audit
      -> review/gate
```
...
| 同一文件被多个 lane 占用 | 先 `/window-handoff-ccgs audit` | 解决 file conflict 后再继续写 |

`/help` 的输出必须同时包含：

- 下一步 Skill。
- 建议在哪个 lane 做。
- 如果 lane 不存在，给出 `/window-start-ccgs <lane-id>`。
- 如果 lane 已存在，给出 `/window-handoff-ccgs <lane-id>` 更新命令。
...
/window-start-ccgs <lane-id>   # 自定义窗口
```

`/window-start-ccgs` 会读取 `AGENTS.md`、`context-management.md`、
...
6. 每个窗口完成一个小阶段后运行 `/window-handoff-ccgs update <lane-id>`。
...
1. **短命令**：更新状态只需要 `/window-handoff-ccgs update B`，不需要手写模板。
2. **固定时机**：在明确触发点必须更新。
3. **可审计**：A-producer 可以运行 `/window-handoff-ccgs audit` 检查哪些 lane 缺失、过期或字段不完整。
...
| 时机 | 命令 | 原因 |
|---|---|---|
| 刚启动并确认职责后 | `/window-handoff-ccgs update <id>` | 记录本窗口正式接手 |
| 完成一个阶段性产物 | `/window-handoff-ccgs update <id>` | 防止成果只留在聊天里 |
| 修改了本窗口 owner 路径 | `/window-handoff-ccgs update <id>` | 记录 active/changed files |
| 产生跨窗口 blocker | `/window-handoff-ccgs update <id>` | 让 A-producer 能协调 |
| 准备关窗口或开新窗口 | `/window-handoff-ccgs update <id>` | 生成恢复点 |
| 上下文达到约 60-70% | `/window-handoff-ccgs update <id>` | 避免压缩卡死后丢状态 |
| lane 文件超过 300 行 | `/window-handoff-ccgs compact <id>` | 控制 MD 长度 |
| A-producer 检查全局状态 | `/window-handoff-ccgs audit` | 查漏、查过期、查 blocker |
...
`/window-handoff-ccgs update <id>` 不是把整份 lane 替换成一段最新摘要。
...
因此，`/window-handoff-ccgs update <id>` 的草稿必须先声明：
...
```text
/window-handoff-ccgs update A
/window-handoff-ccgs update B
/window-handoff-ccgs update C
/window-handoff-ccgs update D
/window-handoff-ccgs update Z
```
...
- 不确定 owner 时，先停止写该文件，运行 `/window-handoff-ccgs audit`。
...
1. 不复制全部对话。
2. 开新窗口。
3. 运行 `/window-start-ccgs [A|B|C|D|Z]`；如果 Skill 不可用，再使用对应 `Restart prompt`。
...
```text
/window-handoff-ccgs compact A
/window-handoff-ccgs compact B
/window-handoff-ccgs compact C
/window-handoff-ccgs compact D
/window-handoff-ccgs compact Z
```
...
4. Z-platform 只在底层工具或 Skill 被修改时运行 `/skill-test audit`。
