...
mkdir -p "$(dirname "$REMINDERS")" 2>/dev/null
{
  ...
  while IFS= read -r skill; do
    [ -n "$skill" ] && echo "- 已修改 Skill: $skill；建议使用 /skill-create-cfg 进行内部验证、路由审计和必要修复。"
  done <<< "$SKILLS"
  echo ""
} >> "$REMINDERS"

{
  echo "检测到 Skill 文件变更："
  while IFS= read -r skill; do
    [ -n "$skill" ] && echo "- $skill：建议使用 /skill-create-cfg 做内部验证和动线接入审计"
  done <<< "$SKILLS"
  echo "批量修改后建议使用 /skill-create-cfg 做 route/catalog/workflow insertion 审计。提醒已写入 $REMINDERS。"
} | emit_system_message
